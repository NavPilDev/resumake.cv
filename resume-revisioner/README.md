...
> **Note:** this is also where [Resumake.cv](../README.md#resumakecv) — a planned single local, Ollama-powered program that bundles this JD-tailoring flow, experience-bank editing, application tracking, and company notes into one tool anyone can run — is being built. It hasn't been renamed yet, so it's still `resume-revisioner` in `package.json`, routes, etc.
...
- **Import from files or pasted text** ([app/components/ExperienceUploadPanel.tsx](app/components/ExperienceUploadPanel.tsx)): upload one or more resume files (PDF/Markdown/plain text/JSON, up to 15MB each) and/or paste free text — each is treated as a separate "attachment." `POST /api/experience/extract` ([app/api/experience/extract/route.ts](app/api/experience/extract/route.ts)) turns each attachment into raw text ([lib/textExtract.ts](lib/textExtract.ts) — PDF via `pdf-parse`, MD/TXT/JSON read directly, capped at ~20k characters), then sends it to your local Ollama model for structured extraction ([lib/experienceExtraction.ts](lib/experienceExtraction.ts)), one attachment at a time. Anything unambiguous merges straight into the live sections below immediately — **there is no separate review-and-accept staging step**; you edit or delete directly in place afterward. Only genuine disagreements between attachments (a contact field with conflicting values, or two attachments describing what looks like the same job/project, matched by exact company+role or project name) are held back for you to resolve, either via inline radio picks or a side-by-side "Compare versions" modal ([app/components/ui/version-compare-modal.tsx](app/components/ui/version-compare-modal.tsx)).
- **Sidebar table of contents** ([app/components/ExperienceSidebarToc.tsx](app/components/ExperienceSidebarToc.tsx)): a collapsible outline of every section/entry with a red-dot indicator on anything missing required fields (e.g. a job with no company or role), so incomplete entries are easy to spot after a bulk import.
- **Manual editing**: every job, project, education entry, and certification is a form row ([app/components/JobEntryRow.tsx](app/components/JobEntryRow.tsx), [ProjectEntryRow.tsx](app/components/ProjectEntryRow.tsx), [EducationEntryRow.tsx](app/components/EducationEntryRow.tsx), [CertificationRow.tsx](app/components/CertificationRow.tsx)) with an "+ Add …" button to create new ones from scratch. Technical skills ([app/components/TechnicalSkillsSection.tsx](app/components/TechnicalSkillsSection.tsx)) work the same way — add/remove categories and individual skills by hand, merged and deduped case-insensitively with whatever Ollama extraction contributed.
