...
Everything here is converging into **Resumeak** — a single local program, powered by Ollama (no data leaves the machine), meant to be run by other people, not just the repo owner. That means code going into `resume-revisioner/` should be written with that eventual standalone-tool audience in mind (e.g. don't hardcode assumptions that only hold for this specific repo's data unless there's no other reasonable way).

- Resumeak's code currently lives in [`resume-revisioner/`](resume-revisioner) — it hasn't been renamed yet, so don't be surprised the app is still called "resume-revisioner" in `package.json`, routes, etc.
- `application-tracker/` and the `latex-*`/`company-notes/` folders are expected to fold into Resumeak over time, but haven't yet — treat them as separate, standalone pieces for now unless told otherwise.
- An **"input your experience"** feature is in progress in `resume-revisioner/` — a UI for building/editing the `experience.yaml` bullet bank from the app itself, instead of hand-editing the YAML file. If you're working in that area, check current app/lib files rather than assuming this README describes the finished feature.
